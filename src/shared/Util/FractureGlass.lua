...
local numCracks = 7 -- Must be at least 4 (wouldn't recommend more than 6) | How many cracks we want
...
local lifetime = 100 -- How many seconds we want the glass to last before getting destroyed
...
return function(part, origin, force, ...)
	...
	if not game.Players.LocalPlayer and replicate and not ... then -- Called on the server
		...
		if canCollide then
			task.delay(anchorDelay + 2, function()
